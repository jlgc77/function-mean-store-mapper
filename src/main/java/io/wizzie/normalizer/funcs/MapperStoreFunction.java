...
/**
 * This class is used to do a mapper process, but it offers a key value store to
 * operate with multiples states
 */
public abstract class MapperStoreFunction implements Function<KeyValue<String, Map<String, Object>>>,
	...
	public void init(ProcessorContext context) {
		if (windownTimeMs != null)
			context.schedule(windownTimeMs, PunctuationType.STREAM_TIME, this::window);
		availableStores.forEach((storeName) -> stores.put(storeName,
				(KeyValueStore) context.getStateStore(String.format("%s_%s", appId, storeName))));
